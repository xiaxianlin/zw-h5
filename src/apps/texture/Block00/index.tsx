...
import { animated, useSpring, useSpringRef, useSpringValue, useTrail } from "@react-spring/web";
import { useEffect, useRef } from "react";

const items = [styles.yang, styles.li, styles.de, styles.song, styles.yun];

export function Block00({ onComplete }: { onComplete?: () => void }) {
  const { loading } = useAppModel();

  const counter = useRef(1);
  const wen = useSpringValue(0);
  const [words, wordsApi] = useTrail(items.length, () => ({ transform: "translateY(800px)" }));
  ...
  const handleTrailFinish = () => {
    ...
    if (counter.current === items.length - 2) {
      ...
      start.start(0, { onRest: onComplete, config: { duration: 1200 } });
    }
  };
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      wordsApi.start({ transform: "translateY(0)", onRest: handleTrailFinish });
    }, 250);
  }, [loading]);
  ...
  return (
    <animated.div className={styles.block}>
      <animated.div className={styles.words}>
        ...
        {words.map(({ transform }, i) => (
          <animated.div key={i} className={items[i]} style={{ transform }} />
        ))}
      </animated.div>
